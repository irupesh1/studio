...
const DefaultLogo = (props: React.SVGProps<SVGSVGElement>) => (
    <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 218 300" preserveAspectRatio="xMidYMid meet" {...props}>
    ...
    } catch (error)
{
  ...
  const ChatAvatar = () => (
    chatAvatar ? 
    <NextImage src={chatAvatar} alt="Chat Avatar" width={24} height={24} className="rounded-full" /> : 
    <DefaultLogo className="text-foreground" />
  );

  return (
    <div className="flex flex-col h-full" style={chatAreaStyle}>
                  ...
                  >
                    ...
                    {message.role === 'assistant' && (
                       <div className="flex items-center gap-4 mt-3">
                          <button onClick={() => handleCopyToClipboard(message.content)} aria-label="Copy message">
                          ...
                          {index === messages.length - 1 && !isLoading && (
                            <button onClick={handleRegenerate} disabled={isLoading} className="disabled:opacity-50" aria-label="Regenerate response">
